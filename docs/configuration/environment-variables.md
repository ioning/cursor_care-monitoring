...
Документация по всем переменным окружения, используемым в системе Care Monitoring.
...
- В production используются Kubernetes Secrets или внешние системы управления секретами
...
EXPO_PUBLIC_APP_NAME=Care Monitoring

# Features
EXPO_PUBLIC_BLE_ENABLED=true
EXPO_PUBLIC_LOCATION_ENABLED=true
...
В production используются следующие изменения:

1. **Секреты**: Все секреты хранятся в Kubernetes Secrets или HashiCorp Vault
2. **Database URLs**: Используются connection pooling и read replicas
3. **Redis**: Используется Redis Cluster для высокой доступности
4. **RabbitMQ**: Используется RabbitMQ Cluster
5. **Logging**: `LOG_LEVEL=warn` для снижения объема логов
6. **Monitoring**: Все метрики отправляются в централизованный Prometheus

## Безопасность

⚠️ **ВАЖНО**: Никогда не коммитьте реальные значения в репозиторий!

- Используйте `.env.example` как шаблон
- Генерируйте сильные секреты для production
