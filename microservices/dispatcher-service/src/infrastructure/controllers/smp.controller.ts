...
export class SMPController {
  ...
  async getProviders(@Request() req, @Query('organizationId') organizationId?: string) {
  ...
  async getServicePrices(@Request() req, @Query('organizationId') organizationId?: string) {
    const orgId = organizationId || req.user?.organizationId;
