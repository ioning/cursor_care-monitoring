...
export class PredictionRepository {
  ...
  async getStats(wardId: string, days: number = 7): Promise<PredictionStats> {
    ...
    severityResult.rows.forEach((row: any) => {
      if (row.severity && Object.prototype.hasOwnProperty.call(severityMap, row.severity)) {
    ...
    if (trendResult.rows.length === 2) {
      const firstHalf = trendResult.rows.find((r: any) => r.period === 'first_half');
      const secondHalf = trendResult.rows.find((r: any) => r.period === 'second_half');
  ...
  async deleteOldPredictions(daysToKeep: number = 90): Promise<number> {
    ...
    const result = await db.query(`DELETE FROM predictions WHERE timestamp < $1 RETURNING id`, [cutoffDate]);
